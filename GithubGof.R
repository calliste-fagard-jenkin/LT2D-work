...
#'}
#'@seealso \code{\link{fityx}} \code{\link{p.kolomogarov}} \code{\link{GoFx}}
#'@export
GoFy=function(fit,plot=FALSE){
  ...
  if(plot){
    ...
    lines(c(0,1),c(1,0))
    mtext(paste('p-values: Cramer-von Mises=',round(p.cvm,2),' ; kolomogarov=',round(p.kolomogarov,2)))
    points(1-e.cdf[worst],cdf[worst],col="red") # mark point on which Kolmogarov test hinges
  }
  return(list(p.cvm=p.cvm,D.kolomogarov=Dn,p.kolomogarov=p.kolomogarov))
}
...
GoFx=function(fit,plot=FALSE,nint=100){
  ...
  if(plot) {
    ...
    lines(c(0,1),c(0,1))
    mtext(paste('p-values: Cramer-von Mises=',round(p.cvm,2),' ; kolomogarov=',round(p.ks,2)))
